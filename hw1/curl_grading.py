...
# 3.3 make cpplint optional until it isnt
...
def test_pystyle(self):
    ...
    pylint_report = proc_pylint.stdout.decode().strip().splitlines()

    if len(pylint_report)<2:
        logging.error('bad pylint_report'+proc_pylint.stdout.decode())
        pylint_score = 0
    elif "previous" in pylint_report[-1]:
        pylint_score=pylint_report[-1].split()[6]
    else:
        pylint_score = pylint_report[-1].split()[-1]
